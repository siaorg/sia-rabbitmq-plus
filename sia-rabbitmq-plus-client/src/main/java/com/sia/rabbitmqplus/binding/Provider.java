...
public class Provider {
	...
	private void sendP2P(Channel channel, String queueName, String message) throws Exception {
		try {
			...
			throw new UnsupportedOperationException("[rabbitMQ服务器上没有名为<" + queueName + ">队列，请联系接收端，让其先启动点对点模式下的消费者]");
		}
	...
	private void send(SIAMessage message, String lable, boolean isChainModel) throws Exception {
		...
		try {
			...
			if (exchangeName != null) {
				...
				if (message.getReceiveQueueName() != null && "beginSend:".equals(lable)) {
					Map<String, Object> args = new HashMap<String, Object>();
					if (message.getExpires() != null) {
						args.put("x-expires", message.getExpires() * 1000); // 队列超时时间30s
					} else {
						args.put("x-expires", 30 * 1000); // 队列超时时间30s
					}
