...
public class ReceviePubSub {
     ...
     * 
     * 比如EXCHANGE_NAME为sia_client_test_send_pubsub，与它绑定的队列有：
     * sia_client_test_receive_message1与sia_client_test_receive_message2， 则这两个队列会接收到同样的消息。
     * 
     * 
     * @param message
     */
    ...
    public static void main(String[] argv) throws Exception {
        ...
        props.load(new ReceviePubSub().getClass().getResourceAsStream("本地路径/conf/log4j.properties"));
        // log4j的配置，我们用slf4j统一接口的，可以指定其他的实现，比如logback
        PropertyConfigurator.configure(props);
        // 指定配置文件的读取位置，默认在class路径下
        PropertyHelper.setProfilePath("本地路径/conf/ReceviePubSub");
        // 启动消费者
        Consumer.start();
    }
