...
var LogHelper={
	...
	debug:function(obj,msg,e) {
		if (this.isdebug) {
			...
			this.fomart("DEBUG",caller,msg,e);
		}
	},
	err:function(obj,msg,e) {
		var caller=this.err.caller;
		this.fomart("ERR",caller,msg,e);
	},
	fomart:function(type,caller,msg,e) {
...
var AjaxHelper={
	call:function(cfg) {
		...
		$.ajax({
			url:cfg.url,
			type:cfg.type,
			async:true,//cfg.async,
			cache:cfg.cache,
			data:cfg.data,
			dataType:cfg.dataType,
			contentType:cfg.contentType,
			beforeSend:function () {
				timeOutId = setTimeout(function(){PageHelper.showLoding();},1000);
		    },
	...
	showLoding : function() {
		try {
			...
			if (lodingDiv) {
				...
				style.append("height:56px;");
				style.append("position: absolute;");
				style.append("top:46%;");
				style.append("left:44.5%;");
				style.append("line-height:56px;");
				style.append("color:#fff;");
				style.append("padding-left:60px;");
				style.append("font-size:15px;");
				style.append("background: #000 url("+loadingUrl+") no-repeat 10px 50%;");
				style.append("opacity: 0.7;");
				style.append("z-index:9999;");
				style.append("-moz-border-radius:20px;");
				style.append("-webkit-border-radius:20px;");
				style.append("border-radius:20px;");
				style.append("filter:progid:DXImageTransform.Microsoft.Alpha(opacity=70);");
				style.append("}");
				//手机样式 
				style.append("@media");
				...
				lodingDiv.setAttribute("class", "lodingDiv");
				lodingDiv.innerHTML = "Loading...";
